...
class RoboAnnotatorMetaForCausalLM(ABC):
    ...
    def hierachical_encoder(self, image_features, prompts=None, image_counts=None): 
        ...
        for _idx in range(len(prompts)):
            if not image_counts: # image-only
                # 1. Visual embedding
                img_feat_prompt = image_features[_idx, None].expand(len(prompts[_idx]), -1, -1) # shape: [prompt_num, image_shape, feat_dim]
                ...
                frame_position_embeddings = self.get_model().video_frame_position_embedding(position_ids) # (prompt_num, frame_num, hidden_size)
                ...
                img_feat_prompt = (img_feat_prompt + frame_position_embeddings.unsqueeze(2)).flatten(0,1) # (prompt_num * frame_num, image_shape, hidden_size)
                ...
                # Query generator
                query_tokens = self.get_model().vlm_att_query.expand(bert_feat.shape[0], -1, -1) # (prompt_num * frame_num, num_query, att_feat) 
                ...
                # 4. Input LLaMA
                final_token = self.token_generation(img_feat_prompt, scene_feature=scene_features, clip_feature=clip_features, motion_feature=motion_features)
                final_token = final_token.reshape(len(prompts[_idx]), -1, final_token.shape[-1])  # shape: [prompt_num, visual_token_number, hidden_feature_dim]
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5: # video/image->video
